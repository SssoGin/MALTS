...
1. **Read runtime docs before initialization.** When initializing MALTS for a new project, read the relevant runtime docs, templates, and checklists before writing project-level `AGENTS.md`. Route Chinese-facing canonical control/report artifacts through the CH templates while preserving stable schema markers and values.
