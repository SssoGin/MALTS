...
7. Before any sub-agent or equivalent dispatch, show the launch review packet: overall goal, total plan, dynamic Agent count, responsibility lanes, requested/recommended/configured/effective model-and-effort evidence, binding status, each task, and each short plan.
8. Wait for the user's explicit `确认运行`.
9. Before each sub-agent dispatch, expose or record the task contract.
10. Record visible OpenCode dispatch evidence, runtime agent ID when available, route evidence reference, effective model/effort when observable, binding status, and recycled feedback in `PROJECT_CONTROL.md`.
