...
1. **Read all runtime docs before /init.** When initializing MALTS for a new project, read all runtime docs (skills, templates, checklists) before writing project-level AGENTS.md. A partial AGENTS.md that misses WORK_TASK_REPORT, Growth Review tiers, or checklist references is harder to fix later.
...
8. Before any sub-agent or equivalent dispatch, show the launch review packet: overall goal, total plan, dynamic Agent count, responsibility lanes, requested/recommended/configured/effective model-and-effort evidence, binding status, each task, and each short plan.
9. Wait for the user's explicit `确认运行`.
10. Before each sub-agent dispatch, expose or record the task contract.
11. Record visible OpenCode dispatch evidence, runtime agent ID when available, route evidence reference, effective model/effort when observable, binding status, and recycled feedback in `PROJECT_CONTROL.md`.
...
The Codex-specific `codex-peer-task` route is not a portable OpenCode provider API. Use OpenCode's own user-visible native dispatch surface and record equivalent task, route, binding, return, acceptance, and closure evidence.
...
- If the user specifies a sub-agent model or effort and the target OpenCode runtime supports it, use the exact runtime ID.
