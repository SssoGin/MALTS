...
- Do not dispatch sub-agents until a launch review is shown and the user replies `确认运行`.
...
When native Codex sub-agent routing cannot satisfy an explicitly requested hard model/effort binding, a user-visible Codex task/thread may be governed as `codex-peer-task`. Prefer the current task workspace, record its task/thread ID and effective route evidence, reuse the same task for rework, prohibit silent fallback, and archive it only after `ACCEPTED` or another terminal closure. The Codex task/thread API is the execution surface; MALTS supplies the governance and evidence contract.
