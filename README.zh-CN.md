...
| 查看安装命令参考 | [安装说明](docs/zh-CN/INSTALL.md) |
| 查看简明使用指南 | [使用指南](docs/zh-CN/USAGE.md) |
| 审阅设计边界 | [核心设计](docs/zh-CN/CORE_DESIGN.md) |
...
- 通过 `WORK_TASK_REPORT.md` 提供阶段和最终交付报告
- 通过 MALTS Memory Pipeline 处理成长复盘和持久候选
- 通过根级 `skills/` 提供唯一 `SKILL.md` 事实源，并安装到各 Agent 工具本地 skill 目录
- 可选的双语文档同步
...
| `WORK_TASK_REPORT.md` | MALTS 阶段或最终交付后使用 | 报告结果、检查、风险和下一步。 |
...
当启用 MALTS，或任务扩大到需要可恢复长任务模式时，在项目根目录创建或复用 `PROJECT_CONTROL.md`。每个 MALTS 阶段或最终交付后，应写入或追加 `WORK_TASK_REPORT.md`。需要未来 Agent 接手时，使用 `PROJECT_HANDOFF.md` 作为记录状态。

| 文件 | 默认角色 |
|---|---|
| `PROJECT_CONTROL.md` | Agent-facing 项目状态和任务队列 |
| `WORK_TASK_REPORT.md` | 面向用户的阶段 / 最终报告，通常跟随用户或项目语言 |
...
完整安装路径见 [docs/zh-CN/GETTING_STARTED.md](docs/zh-CN/GETTING_STARTED.md) 和 [docs/INSTALL.md](docs/INSTALL.md)。
...
docs/zh-CN/              Public Simplified Chinese documentation
```
