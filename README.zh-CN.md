...
- 通过 `WORK_TASK_REPORT.md` 提供阶段和最终交付报告；中文用户或双语模式下同步维护 `工作任务报告.md`
...
| `WORK_TASK_REPORT.md` / `工作任务报告.md` | MALTS 阶段或最终交付后使用 | 报告结果、检查、风险和下一步。中文用户或双语模式下必须保持成对维护。 |
...
当启用 MALTS，或任务扩大到需要可恢复长任务模式时，在项目根目录创建或复用 `PROJECT_CONTROL.md`。中文用户或双语项目模式下，同时创建和维护 `项目控制.md`。每个 MALTS 阶段或最终交付后，应写入或追加 `WORK_TASK_REPORT.md`；中文用户可读输出在范围内时，同步写入或追加 `工作任务报告.md`。需要未来 Agent 接手时，使用 `PROJECT_HANDOFF.md` 作为记录状态。

| 文件 | 默认角色 |
|---|---|
| `PROJECT_CONTROL.md` | Agent-facing 项目状态和任务队列 |
| `项目控制.md` | 中文用户可读的项目控制镜像 |
| `WORK_TASK_REPORT.md` | Agent-facing 结构源和阶段 / 最终报告源 |
| `工作任务报告.md` | 中文用户可读的阶段 / 最终报告 |
| `PROJECT_HANDOFF.md` | Agent-facing 继续和恢复事实源 |
...
公开仓库默认以英文源文档作为 Agent 执行事实源。简体中文公开文档位于 `docs/zh-CN/`，中文运行模板镜像位于 `runtime/CH/`。当中文用户可读输出或双语模式在范围内时，双语运行产物对是规范要求；普通执行仍应避免同时加载两种语言，除非任务明确要求中文输出、校对或同步。双语维护规则见 [docs/zh-CN/BILINGUAL_DOCS.md](docs/zh-CN/BILINGUAL_DOCS.md)。
