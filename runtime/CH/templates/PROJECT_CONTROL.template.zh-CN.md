...
> 对应 Agent-facing 项目状态源是 `PROJECT_CONTROL.md`。中文用户可读状态在范围内时，维护 `项目控制.md` 作为实质镜像。  
...
当项目同时保留 Agent 读取的英文控制文件和用户阅读的中文控制文件时，使用本节。

- Agent 读取控制文件：`PROJECT_CONTROL.md`
- 用户阅读控制文件：`项目控制.md`
- 事实源策略：英文给 Agent 运行时读取；中文给用户审阅。
- 同步规则：实质状态变化必须同步到两份文件，或记录某一侧不适用的原因。
- 最近同步检查：
