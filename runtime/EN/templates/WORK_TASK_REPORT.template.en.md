# WORK_TASK_REPORT

> Purpose: Agent-facing structure and evidence report after each completed MALTS task or phase. When Chinese user-facing output or bilingual mode is in scope, maintain the paired `工作任务报告.md` mirror from `runtime/CH/templates/WORK_TASK_REPORT.template.zh-CN.md`.

## Result

- Status: Completed / Partially Completed / Blocked / Failed
- Plain-language conclusion:
- User original goal addressed:
- Paired Chinese report required: Yes / No; reason:
- Paired Chinese report path: `工作任务报告.md` / N/A

## What Was Done

- Main changes:
- Files or deliverables created/updated:
- Artifact/directory boundary, if any:
- Grill-Me Preflight: offered / accepted / declined / N/A; decisions resolved:
- Multi-agent roles used, if any:

## Verification

- Checks run:
- Result:
- Evidence level:
- Not verified:
- Delivery checklist reviewed: Yes / No / N/A; reason:

## Cost And Model Strategy

- Documentation sync strategy: script-first / low-cost candidate generation / high-capability approval / N/A
- Source files, target files, and sync direction:
- Low-cost model/agent candidate work performed:
- Script or structured check result:
- High-capability/main-controller approval scope:
- Final status if approval is missing: Draft / Unverified / N/A
- Runtime model-routing limitation, if any:
- Unreviewed risk:

## Growth Review

- Review level: None / Light / Standard / Major
- Reusable experience found: Yes / No
- What changed for next time:
- Promotion decision: None / Local only / Proposed for GLOBAL_MEMORY / Written to GLOBAL_MEMORY
- Long-term memory decision: Do not write / Local candidate / Proposed global rule / Written after checklist
- Destination or record location:
- GLOBAL_MEMORY entry ID or fallback location:
- Lifecycle check: <any GLOBAL_MEMORY entries ready for promotion? experimental→stable after 3+ verified uses. stable→deprecated after 90 days unverified.>

## MALTS Memory Pipeline

- Pipeline action: None / Local candidate / Work task report / PROJECT_CONTROL / Proposed GLOBAL_MEMORY / Written after checklist / Optional external memory
- Content recorded: <summary of filtered key decisions or growth results>
- Destination or fallback:
- Reason if skipped:

## Continuity And Recovery

- Was `PROJECT_CONTROL` or equivalent state updated: Yes / No / N/A
- Was `项目控制.md` updated when required: Yes / No / N/A
- Was `工作任务报告.md` updated when required: Yes / No / N/A
- Round completed:
- Runtime duration note: no fixed one-shot runtime guarantee / N/A
- Recovery point:
- New-window or other-project resume path:
- If context was compacted or interrupted, how work can continue:
- Unattended auto-continue status: Not Authorized / Authorized / Running / Stopped / Completed / N/A
- Next round stop condition:

## Risks And Limits

- Known risks:
- Remaining work:
- Decisions needed from user:

## Where To Look

- Primary file or report:
- Supporting files:

## Next Step

- Recommended next action:
