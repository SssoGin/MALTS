...
| 新窗口、中断或上下文丢失后难以恢复工作。 | 用 `PROJECT_CONTROL.md` 记录状态，用 `WORK_TASK_REPORT.md` 记录阶段结果，用 `PROJECT_HANDOFF.md` 记录继续上下文。 |
...
|---|---|---|
| Codex | `AGENTS.md` | 共享 skills 从根级 `skills/` 安装 |
...
应确保 Agent 能读取 MALTS 仓库，或把需要的共享 skills、templates 和 checklists 复制到 Agent 可读位置。根级 `skills/` 是唯一 skill 事实源。`runtime/EN/templates` 和 `runtime/EN/checklists` 继续作为模板和检查清单来源。中文文档是公开入口和译本，不是默认 Agent runtime 事实源。
...
7. 如果另一个 Agent 或新窗口需要继续，更新 `PROJECT_HANDOFF.md`。
