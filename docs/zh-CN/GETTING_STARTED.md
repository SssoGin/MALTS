...
安装器会创建一份共享 MALTS root，并在每个选中的工具目录写入 `MALTS_BOOT.md`。安装后，确认 `MALTS_BOOT.md` 能解析到包含 `README.md`、`skills/`、`runtime/EN/templates` 和 `runtime/EN/checklists` 的 root。共享 `MALTS_ROOT` 下的 `skills/` 是唯一默认 skill 事实源。`runtime/CH` 下的中文运行镜像用于中文用户可读报告和双语同步；普通英文 Agent 执行不应加载中文镜像，除非当前任务需要。
