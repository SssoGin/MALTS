...
公开包只有一个 canonical skill 实现和 runtime source：共享 `MALTS_ROOT`，并以该 root 下的 `skills/` 为准。目标工具目录是薄 adapter target，不是独立 MALTS runtime 或实现 source；其中可以包含工具原生索引所需的发现型 bridge `SKILL.md` 和 `agents/openai.yaml` UI 元数据。MALTS 拥有的 bridge 目录名和 front matter name 必须使用 `malts-*` 前缀，同时仍路由到共享 root 下的 canonical skill 路径；这样可以保证 `/MALTS` 搜索可靠，不需要重命名实现事实源。面向用户的 bridge 元数据必须显示全大写 `MALTS ...` 名称，短描述不得重复标题，避免原生 skill 选择器出现重复和大小写漂移。`runtime/EN` 包含 MALTS workflows 使用的 Agent-facing templates 和 checklists。`runtime/CH` 包含简体中文运行镜像，用于用户可读审阅和双语产物生成。

### Capability 治理边界

v1.0 Capability Registry 是 canonical source 与工具 overlay 的元数据视图，不是另一个物理 Skill repository。物理可移植性、工具兼容性和 catalog exposure 是三个独立决策。Workflow Router 始终只读且只提供建议，不能修改 discovery state、调用 Skill 或绕过授权。事务 lifecycle 只激活经过验证的 MALTS-owned 内容与 projection，不会成为第三方 Skill manager。完整契约见 [Capability 与 Skill 治理](CAPABILITY_AND_SKILL_GOVERNANCE.md)。
...
初始化输出必须列出 active Phase、当前是否存在 Session、没有创建 Session 的原因和下一项 bounded operation，使用户看到的完成结论与持久化 recovery state 一致。
...
Main Controller 是每个 multi-agent round 必需的责任所有者。下表中的其他行都是可选的责任 lane，不是必须具备的角色，也不是固定执行顺序。应根据实际工作、已批准范围、无冲突 ownership 与 effective runtime capacity 选择零个、一个或 N 个额外 lane。角色只描述责任，不能据此推断 model capability 或 reasoning effort。
...
- 通过 `MALTS_BOOT.md` 和共享 `MALTS_ROOT` 解析 MALTS skills 与 runtime files。
- 工具本地只安装 `malts-*` 发现 bridge 及其 UI 元数据，不安装完整 skill 实现重复源。
...
除非变更明确 scoped to one tool，否则 adapter docs 和 templates 应在 Codex、Claude Code 和 OpenCode 之间保持同步。公开 protocol 变更也应同步 EN+CH counterparts，除非 release note 明确记录某个语言被跳过的原因。

顶层工具指令文件属于混合所有权。MALTS 只拥有 `<!-- MALTS:BEGIN managed instruction -->` 与 `<!-- MALTS:END managed instruction -->` 之间的区块，周围文本全部归用户所有。安装器和更新器的默认行为必须幂等合并该区块、迁移可明确识别的旧 MALTS section，并在标记有歧义时停止。整份文件替换属于显式操作，绝不能作为安全更新默认值。已安装工具指令可用 `check-managed-instruction-sync` 对照 adapter source 检查 managed block 是否同步。
...
2. Core skills：Grill-Me Preflight、long-project workspace initialization、long-task scheduling、project retrospective growth、session handoff、project initialization，以及 root `skills/` 下的 single-agent low-overhead growth。
