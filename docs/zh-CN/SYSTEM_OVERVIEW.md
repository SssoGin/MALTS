...
根级 `skills/` 是 MALTS `SKILL.md` workflow 的唯一事实源，包含 Grill-Me Preflight、multi-agent scheduling、session handoff、retrospective growth、lightweight single-agent growth 和 project initialization。安装脚本会把这些 skills 复制到各受支持工具的本地 skill 目录。
...
| Adapter instruction templates | 可选 | 受支持 Agent 工具需要记住 MALTS 行为 | 必须先审阅并与现有本地指令合并 |
