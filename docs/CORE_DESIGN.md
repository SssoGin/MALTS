...
The public package has one canonical skill implementation and runtime source: the shared `MALTS_ROOT`, with `skills/` at that root. Target tool directories are thin adapter targets, not separate MALTS runtime or implementation sources. They may contain discovery-only bridge `SKILL.md` files required by native tool indexing plus `agents/openai.yaml` UI metadata. MALTS-owned bridge directory names and front matter names must use the `malts-*` prefix while still routing to the canonical shared skill path; this keeps `/MALTS` search reliable without renaming the implementation source. User-facing bridge metadata must show uppercase `MALTS ...` display names and short descriptions that do not repeat the title, so native skill pickers avoid duplicate mixed-case labels. `runtime/EN` contains Agent-facing templates and checklists used by MALTS workflows. `runtime/CH` contains Simplified Chinese runtime mirrors for user-facing review and bilingual artifact generation.

### Capability Governance Boundary

The v1.0 Capability Registry is a metadata view over canonical sources and tool overlays, not another physical Skill repository. Physical portability, tool compatibility, and catalog exposure are separate decisions. The Workflow Router remains read-only and advisory; it cannot mutate discovery state, invoke a Skill, or bypass authorization. The transactional lifecycle activates only verified MALTS-owned content and projections; it does not become a third-party Skill manager. The full contract is defined in [Capability And Skill Governance](CAPABILITY_AND_SKILL_GOVERNANCE.md).
...
Initialization output must name the active Phase, state whether a Session exists, explain why no Session was created, and identify the next bounded operation. This makes the user-visible completion claim agree with the persisted recovery state.
...
- resolve MALTS skills and runtime files through `MALTS_BOOT.md` and the shared `MALTS_ROOT`
- install only `malts-*` discovery bridges and their UI metadata locally; never install tool-local skill implementation duplicates
...
2. Core skills: Grill-Me Preflight, long-project workspace initialization, long-task scheduling, project retrospective growth, session handoff, project initialization, and single-agent low-overhead growth under root `skills/`.
...
Ordinary runtime discovery starts from one strict tool-adjacent `MALTS_BOOT.md` `MALTS_ROOT:` line. The lifecycle registry, sole active record, exact active pointer, `VERSION`, and generation identity must agree. MALTS v1.1.1+ does not use a machine-global `GLOBAL_BOOT.md`. Split brain fails closed; ordinary discovery remains read-only and avoids a full-tree hash.
